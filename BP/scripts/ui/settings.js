...
export function settings(sender) {
    system.run(() => {
        ...
        settings.show(sender).then((r) => {
            ...
            if (!r.formValues[10]) {
                sendMessage('risiko.settings.save', { name: sender.name });
            ...
            if (r.formValues[10]) {
                risikoSave.settings = {}
                sendMessage('risiko.settings.deleted', { name: sender.name });
            }

            if (r.formValues[11]) {
                risikoSave.player = {}
                sendMessage('risiko.players.deleted', { name: sender.name });
            }

            if (r.formValues[12]) {
                risikoSave.kingdom = {}
                sendMessage('risiko.kingdom.deleted', { name: sender.name });
            }
